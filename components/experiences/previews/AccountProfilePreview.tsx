...
export function AccountProfilePreview({
  ...
  const isSidebar = layoutStyle === 'sidebar' && device !== 'mobile';
  const contentLayoutClass = isSidebar ? 'grid grid-cols-[1.2fr_0.8fr] gap-3' : 'space-y-4';
  const quickActionGridClass = device === 'desktop' ? 'grid grid-cols-3 gap-2 text-xs' : 'grid grid-cols-1 gap-2 text-xs';
